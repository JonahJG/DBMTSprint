-- Table: public.Compost

-- DROP TABLE IF EXISTS public."Compost";

CREATE TABLE IF NOT EXISTS public."Compost"
(
    "CompostID" integer NOT NULL,
    "EventID" integer NOT NULL,
    "AddressID" integer NOT NULL,
    "Weight" numeric NOT NULL,
    CONSTRAINT "Compost_pkey" PRIMARY KEY ("CompostID"),
    CONSTRAINT "AddressID" FOREIGN KEY ("AddressID")
        REFERENCES public."Address" ("AddressID") MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
        NOT VALID,
    CONSTRAINT "EventID" FOREIGN KEY ("EventID")
        REFERENCES public."Events" ("EventID") MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
        NOT VALID
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS public."Compost"
    OWNER to postgres;